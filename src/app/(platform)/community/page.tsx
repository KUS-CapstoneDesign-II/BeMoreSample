    ...
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      ...
      <Card className="p-6 space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <Button aria-label="글 올리기" className="px-3" onClick={()=>{
            if (!text.trim()) return;
      ...
      <div className="space-y-3">
        ...
        {posts.map(p=> (
          <Card key={p.id} className="p-6 text-sm flex items-center justify-between bg-gradient-to-b from-secondary/10 to-background">
            ...
            <Button size="sm" variant="ghost" onClick={()=>{ likeCommunityPost(p.id); setPosts(listCommunityPosts(topic)); }}>공감 {p.likes ?? 0}</Button>
          </Card>
        ))}
      </div>
    </div>
